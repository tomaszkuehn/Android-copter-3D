// Apache Helicopter FBX Viewer with Animated Rotors (C) 2025 by T.Kuehn @ AI
...
// Get base path for assets (works on both web and Android)
function getBasePath() {
    // Check if running in Capacitor Android
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
        // On Android, assets are in the root of the served content
        return './';
    }
    return './';
}
...
async function loadTerrainOptions() {
    try {
        ...
        // Use XMLHttpRequest for better Android WebView compatibility
        if (window.Capacitor && window.Capacitor.isNativePlatform()) {
            text = await new Promise((resolve, reject) => {
                const xhr = new XMLHttpRequest();
                xhr.open('GET', basePath + 'terrains.txt');
                xhr.onload = () => resolve(xhr.responseText);
                xhr.onerror = () => reject(new Error('Failed to load terrains.txt'));
                xhr.send();
            });
        } else {
            const response = await fetch(basePath + 'terrains.txt');
...
function createGround(ground_name) {
    ...
    const diffTexture = textureLoader.load(basePath + folderName + '.blend/textures/' + texturePrefix + '_diff_4k.jpg');
